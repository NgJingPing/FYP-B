"""
This anpr_detection_entry python files is detecting vehicle, detecting licence plate, recognising number plate, 
recording into entrylog database when the system result is "found" and recording into deniedaccess database 
when the system result is "not found". The camera is "entry", the temporary entrylog images will store in 
entrylogtemp folder and the permanent entrylog images will store in entrylog folder inside the images folder. 
                        ...
                        for i in indices_2:
                            ...
                            if file_exists == True:
                                try:
                                    ...
                                    if camera.lower() == "entry":
                                        save_plate = 'Naim Vision\ANPR\entrylogtemp\saved3.jpg'
                                    elif camera.lower() == "exit":
                                        save_plate = 'Naim Vision\ANPR\exitlogtemp\saved3.jpg'
                                    ...
                                    with open(save_plate, 'rb') as fp:
                                        response = requests.post(
                                            ...
                                            headers={'Authorization': 'Token b5571e18155117e2dbe83dc8aee73623b835dd31'})
                                    #pprint(response.json())
                                    data = response.json()
                                    s = (data['results'][0]['plate'])
                                    plate_num = str(s).upper()
